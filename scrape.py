        ...
        for i in range(len(matchIDs)):
            ...
            for tableCover in soup.find_all('table', class_='wf-table-inset mod-econ')[1::2]:
                ...
                for td in table[1:]:
                    ...
                    roundEcos.append([bank.text.strip()[:-1] for bank in td.find_all('div', class_='bank')])
                    roundSpent.append([r.text.count("$") for r in Round])
                        
                file.write(str([teamNames, roundWins, roundEcos, roundSpent]))
                file.write(",\n")

            time.sleep(2 + 2*random.random())
            print(f"{year}: {len(matchIDs)-i-1} matches left")
        file.write("]\n")
